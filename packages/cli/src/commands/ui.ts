 ...
 */
export const FALLBACK_PROMPT = `You are generating a single-file HTML dashboard for Chorus, a local workflow
...
Output a single dashboard.html file. NO build step, NO npm, NO external CDN
(not even Tailwind's CDN, not even Google Fonts). Must work offline. Inline
CSS and JS. Vanilla fetch + DOM manipulation. No frameworks. Under 500 lines.
Should open in a browser via file:// or be served by any static server.
...
- Timestamps: show as "5 min ago" / "2 hours ago" / "Apr 14" — never raw
  ISO8601. Include the ISO as a title= tooltip so power users can hover.
...
information-dense design: system font stack, monospace for ids/hashes,
subtle cool-gray background, high-contrast foreground, no rounded-corner
chrome, no shadows. Think "inspector panel," not "marketing site."
