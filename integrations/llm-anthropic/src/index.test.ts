...
/**
 * Build an OperationContext with the given credentials. When `mockModel` is
 * supplied, we stash it under a reserved `_providerOverride` key that the
 * integration recognises as "skip provider construction, use this instead."
 */
...
describe("@delightfulchorus/integration-llm-anthropic module shape", () => {
  it("exports a valid IntegrationModule", () => {
    expect(integration.manifest.name).toBe("llm-anthropic");
  ...
  it("declares an apiKey credentialType with a fields catalog", () => {
    ...
    expect(ct.name).toBe("anthropicApiKey");
    expect(ct.authType).toBe("apiKey");
    expect(ct.documentationUrl).toMatch(/^https:\/\/console\.anthropic\.com/);
    expect(ct.fields).toHaveLength(1);
    const field = ct.fields![0]!;
    expect(field.name).toBe("apiKey");
    expect(field.type).toBe("password");
    expect(field.pattern).toBe("^sk-ant-");
    expect(field.deepLink).toBe("https://console.anthropic.com/settings/keys");
  });
...
describe("generate — happy path", () => {
  it("returns text + usage + finishReason from the provider", async () => {
    ...
    expect(snapshot.calls[0]!.key).toBe("llm-anthropic.generate.200");
  });
  ...
  it("passes system prompt + temperature + maxTokens through", async () => {
    ...
    const mockModel = new MockLanguageModelV1({
      doGenerate: async (args: unknown) => {
        ...
        // AI SDK converts `system` into a system message at the front of `prompt`.
...
describe("generate — auth + errors", () => {
  ...
  it("maps provider 401 to AuthError", async () => {
    const mockModel = new MockLanguageModelV1({
      doGenerate: async () => {
        const e = new Error("invalid x-api-key") as Error & { statusCode?: number };
        e.statusCode = 401;
        throw e;
      },
    });
...
describe("generateObject — happy path", () => {
  it("returns structured object matching the schema", async () => {
    ...
    expect(out.object).toEqual({ name: "alice", age: 30 });
    expect(out.usage.inputTokens).toBe(10);
    expect(out.usage.outputTokens).toBe(20);
    expect(out.finishReason).toBe("stop");
    expect(snapshot.calls[0]!.key).toBe("llm-anthropic.generateObject.200");
  });
});
...
describe("testCredential", () => {
  it("returns ok:true when a minimal call succeeds", async () => {
    ...
    const result = await testCredential("anthropicApiKey", makeContext({ mockModel }));
...
describe("extractApiKey", () => {
  ...
  it("accepts { api_key }", () => {
    expect(extractApiKey({ api_key: "sk-ant-x" })).toBe("sk-ant-x");
  });

  it("accepts { token }", () => {
    expect(extractApiKey({ token: "sk-ant-x" })).toBe("sk-ant-x");
  });
...
describe("mapProviderError", () => {
  ...
  it("preserves retryAfterMs when already in ms", () => {
    const e = new Error("429") as Error & { statusCode?: number; retryAfterMs?: number };
    e.statusCode = 429;
