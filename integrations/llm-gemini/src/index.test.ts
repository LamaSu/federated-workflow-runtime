...
function makeContext(opts: {
  ...
  const baseCreds =
    "credentials" in opts ? opts.credentials : { apiKey: "AIzaSyTEST_KEY_FOR_TESTS" };
...
describe("@delightfulchorus/integration-llm-gemini module shape", () => {
  it("exports a valid IntegrationModule", () => {
    expect(integration.manifest.name).toBe("llm-gemini");
  ...
  it("declares an apiKey credentialType with a fields catalog", () => {
    ...
    expect(ct.name).toBe("googleAiStudioApiKey");
    expect(ct.authType).toBe("apiKey");
    expect(ct.documentationUrl).toMatch(/^https:\/\/aistudio\.google\.com/);
    expect(ct.fields).toHaveLength(1);
    const field = ct.fields![0]!;
    expect(field.name).toBe("apiKey");
    expect(field.type).toBe("password");
    expect(field.deepLink).toBe("https://aistudio.google.com/app/apikey");
  });
...
describe("generate — happy path", () => {
  it("returns text + usage + finishReason from the provider", async () => {
    ...
    expect(snapshot.calls[0]!.key).toBe("llm-gemini.generate.200");
  });
...
describe("generate — auth + errors", () => {
  ...
  it("maps provider 401 to AuthError", async () => {
    const mockModel = new MockLanguageModelV1({
      doGenerate: async () => {
        const e = new Error("API_KEY_INVALID") as Error & { statusCode?: number };
        e.statusCode = 401;
        throw e;
      },
    });
  ...
  it("maps provider 403 to AuthError (Gemini permission-denied)", async () => {
    const mockModel = new MockLanguageModelV1({
      doGenerate: async () => {
        const e = new Error("PERMISSION_DENIED") as Error & { statusCode?: number };
        e.statusCode = 403;
        throw e;
      },
    });
  ...
  it("maps provider 429 to RateLimitError with retryAfterMs when present", async () => {
    const mockModel = new MockLanguageModelV1({
      doGenerate: async () => {
        const e = new Error("RESOURCE_EXHAUSTED") as Error & {
          statusCode?: number;
          retryAfter?: number;
        };
        e.statusCode = 429;
  ...
  it("returns structured object matching the schema", async () => {
    const mockModel = new MockLanguageModelV1({
      ...
      doGenerate: async () => ({
        ...
        text: JSON.stringify({ name: "carol", age: 28 }),
      }),
    });
    ...
    expect(out.object).toEqual({ name: "carol", age: 28 });
    expect(out.usage.inputTokens).toBe(10);
    expect(out.usage.outputTokens).toBe(20);
    expect(out.finishReason).toBe("stop");
    expect(snapshot.calls[0]!.key).toBe("llm-gemini.generateObject.200");
  });
});
...
describe("extractApiKey", () => {
  ...
  it("accepts { apiKey }", () => {
    expect(extractApiKey({ apiKey: "k" })).toBe("k");
  });

  it("accepts { api_key }", () => {
    expect(extractApiKey({ api_key: "k" })).toBe("k");
  });

  it("accepts { token }", () => {
    expect(extractApiKey({ token: "k" })).toBe("k");
  });
...
describe("mapProviderError", () => {
  ...
  it("throws AuthError for Gemini PERMISSION_DENIED message even without statusCode", () => {
    const e = new Error("PERMISSION_DENIED: the provided key lacks permission");
    expect(() => mapProviderError(e, "x", "y")).toThrow(AuthError);
  });
  ...
  it("throws RateLimitError for Gemini RESOURCE_EXHAUSTED message", () => {
    const e = new Error("RESOURCE_EXHAUSTED: quota exceeded");
    try {
      mapProviderError(e, "x", "y");
      throw new Error("should have thrown");
    } catch (err) {
